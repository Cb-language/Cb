...
protected:
    ...
    Stmt(const Token& token, Scope* scope, ClassNode* currClass);
    Stmt(const Token& token, Scope* scope, IFuncDeclStmt* funcDecl, ClassNode* currClass);
...
inline Stmt::Stmt(const Token& token, Scope* scope, ClassNode* currClass) : token(token), scope(scope), currClass(currClass)
{
    funcDecl = nullptr;
}

inline Stmt::Stmt(const Token& token, Scope* scope, IFuncDeclStmt* funcDecl, ClassNode* currClass) : token(token), scope(scope), funcDecl(funcDecl), currClass(currClass)
{
}
